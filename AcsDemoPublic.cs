...
namespace HikvisionGetUsers
{
    ...
    public class AcsDemoPublic
    {
        ...
        public static void WriteBytesToFile(byte[] bytes, string strPath)
        {
            try
            {
                using (FileStream fs = new FileStream(strPath, FileMode.OpenOrCreate))
                {
                    Debug.Assert(File.Exists(strPath));
            ...
            catch (Exception e)
            {
                ...
                Console.WriteLine("Write file exception details{}" + e.Data.ToString());
            }
        }
    }
    ...
    {
        public ISAPIRemoteConfig(Int32 UserID, int commandCode, string url)
        {
            this.url = url;
